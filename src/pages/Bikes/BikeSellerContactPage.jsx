...
import './BikeSellerContactPage.css';

const BikeSellerContactPage = () => {
  ...
  useEffect(() => {
    // Load advert data from localStorage
    ...
    if (storedData) {
      ...
      if (parsed.advertData?.contactEmail) {
        setFormData(prev => ({
          ...prev,
          email: parsed.advertData.contactEmail
        }));
      }
      if (parsed.advertData?.contactPhone) {
        setFormData(prev => ({
          ...prev,
          phoneNumber: parsed.advertData.contactPhone
        }));
      }
      if (parsed.advertData?.location) {
        setFormData(prev => ({
          ...prev,
          postcode: parsed.advertData.location
        }));
      }
    } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Update advert data with contact details
      ...
      // Navigate to bike advertising prices page with all data
      navigate('/bikes/advertising-prices', {
        state: {
          advertId,
          advertData: updatedAdvertData,
          vehicleData,
  ...
  // Handle trade dealer publish (bypasses payment)
  const handlePublishVehicle = async () => {
    ...
    try {
      ...
      // Clear localStorage
      localStorage.removeItem(`bikeAdvert_${advertId}`);

      // Navigate to success or dashboard
      navigate('/trade/dashboard', {
        state: {
          message: `Your ${vehicleData?.make || 'bike'} ${vehicleData?.model || ''} has been published successfully!`
        }
      });
    } catch (error) {
      console.error('Publish error:', error);
      setPublishError(error.message || 'Failed to publish bike. Please try again.');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="bike-seller-contact-page">
      {/* Header */}
      <div className="page-header">
        <div className="header-container">
          <div className="header-left">
            <button
              className="back-link"
              onClick={() => navigate(`/bikes/selling/advert/edit/${advertId}`)}
            >
              ← Back to advert
            </button>
      ...
      <div className="main-container">
        <div className="contact-form-container">
          <p className="form-intro">
            ...
            get in touch with you about your bike.
          </p>
          ...
          {publishError && (
            <div className="error-message publish-error">
              {publishError}
            </div>
          )}

          {/* Submit Button - Different for trade dealers vs private sellers */}
          ...
          {isDealerAuthenticated && dealer && (
            <p className="trade-info">
              As a trade dealer, your bike will be published immediately without payment.
            </p>
          )}
        </div>
      </div>
...
export default BikeSellerContactPage;
